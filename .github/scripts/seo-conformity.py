...
Shared with the Weasis documentation site (nroduit.github.io), where it came
from: keep the two copies in step rather than letting them drift.
...
a few template overrides, and it breaks silently. A single flag can make the best landing
pages `noindex` while the sitemap keeps advertising them; the theme appends
`index.html` to internal links, which quietly disagrees with every canonical; /llms.txt reaches the site root through a `../` in an output format's
...
The invariants, roughly in order of how likely each is to break:
  ...
  * every indexable page has a title, a description (warned about below 70 or above
    ...
    fits the two lines its fixed font size was chosen for, and no
...
# Pages kept out of the sidebar with `hidden: true` that must nevertheless stay
# indexable and listed. This site has none today; the check is kept so that the
# day one appears, naming it here is all that is needed.
SHORTCUT_PAGES = ()

# Pages that llms.txt is allowed not to mention. None on this site: the map
# covers the whole English tree.
LLMS_COVERAGE_EXEMPT = ()
...
# longest title on the Weasis site, whose heading size this one shares; the
# longest here is 51. A longer one wraps to a third line and towers over the
# page, which is the thing that size was chosen to avoid. Change the values
# there and this number together, on both sites.
H1_TWO_LINES = 58
